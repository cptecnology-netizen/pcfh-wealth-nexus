  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-slate-200/60 shadow-lg hover:shadow-xl transition-all duration-300 group">
          <div className="flex items-center justify-between mb-4">
            <span className="text-slate-500 text-sm font-medium">Património Líquido Total</span>
            <div className="p-2 bg-blue-50 text-blue-600 rounded-xl group-hover:scale-110 transition-transform duration-300">
              <Wallet size={20} />
            </div>
          </div>
          <div className="text-3xl font-bold text-slate-900 tracking-tight">{CURRENCY_FORMATTER.format(totalWealth)}</div>
          <div className="flex items-center mt-2 text-emerald-600 text-sm font-medium bg-emerald-50 px-2 py-1 rounded-full w-fit">
            <TrendingUp size={14} className="mr-1" />
            <span>+4.5% (YTD)</span>
          </div>
        </div>

        <div className={`p-6 rounded-2xl border shadow-lg hover:shadow-xl transition-all duration-300 backdrop-blur-md bg-white/80 ${isReserveLow ? 'border-red-200/60' : 'border-slate-200/60'}`}>
          <div className="flex items-center justify-between mb-4">
            <span className="text-slate-500 text-sm font-medium">Cobertura Reserva Fiscal</span>
            <div className={`p-2 rounded-xl transition-transform duration-300 hover:scale-110 ${isReserveLow ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
              <AlertTriangle size={20} />
            </div>
          </div>
          <div className="text-3xl font-bold text-slate-900 tracking-tight">{reserveCoverage.toFixed(1)}%</div>
          <div className="flex items-center mt-2 text-slate-500 text-sm">
            <span>Meta: 105% (Obrigação + 5%)</span>
          </div>
          {isReserveLow && (
            <div className="mt-3 animate-pulse">
              <span className="text-[10px] uppercase tracking-wider text-red-600 font-bold bg-red-100 px-2 py-1 rounded-md">
                Ação Requerida
              </span>
              <p className="text-xs text-red-700 mt-2 font-medium">
                Défice de <strong>{CURRENCY_FORMATTER.format(shortfall)}</strong>
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-slate-200/60 shadow-lg hover:shadow-xl transition-all duration-300 group">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="p-2 bg-emerald-50 text-emerald-600 rounded-xl group-hover:scale-110 transition-transform duration-300">
              <HeartHandshake size={20} />
            </div>
          </div>
          <div className="text-3xl font-bold text-slate-900 tracking-tight">{CURRENCY_FORMATTER.format(17000000)}</div>
          <div className="flex items-center mt-2 text-slate-500 text-sm">
        ...
        <div className="bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-slate-200/60 shadow-lg hover:shadow-xl transition-all duration-300 group">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="p-2 bg-orange-50 text-orange-600 rounded-xl group-hover:scale-110 transition-transform duration-300">
              <TrendingDown size={20} />
            </div>
          </div>
          <div className="text-3xl font-bold text-slate-900 tracking-tight">Médio</div>
          <div className="flex items-center mt-2 text-orange-600 text-sm font-medium bg-orange-50 px-2 py-1 rounded-full w-fit">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="space-y-4">
            {MOCK_ALERTS.map(alert => (
              <div key={alert.id} className={`p-4 rounded-lg border-l-4 ${alert.severity === 'warning' ? 'bg-orange-50 border-orange-400' :
                  alert.severity === 'error' ? 'bg-red-50 border-red-500' :
                    'bg-blue-50 border-blue-400'
                }`}>
                <p className={`text-sm font-semibold ${alert.severity === 'warning' ? 'text-orange-800' :
